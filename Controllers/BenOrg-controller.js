...
const registerBenOrg = async (req, res) => {
    ...
    try {
        ...
        const existingUser = await BenOrgMODEL.findOne({ email });
...
const loginBenOrg = async (req, res) => {
    ...
    try {
        ...
        const user = await BenOrgMODEL.findOne({ email });
...
const sendEmailBenOrg = async (req, res) => {
    try {
        const email = req.body.email;
        emailFromSendEmail = email;

        const user = await BenOrgMODEL.findOne({ email: email });
        if (user) {
...
const verificationCodeBenOrg = async (req, res) => {
...
const updatepasswordBenOrg = async (req, res) => {
    ...
    try {
        ...
        const result = await BenOrgMODEL.findOneAndUpdate(
            { email: email },
...
const getBenOrgData = async (req, res) => {
    try {
        const users = await BenOrgMODEL.find({ isDeleted: false });
        res.json(users);
    } catch (err) {
        console.error(err);
...
const getBenOrgId = async (req, res) => {
    const { id } = req.params;
    try {
        const user = await BenOrgMODEL.findOne({ _id: id, isDeleted: false });
...
const updateBenOrgData = async (req, res) => {
    const { id } = req.params;
    const { orgName, orgId, address, email, password, confirm_password, phoneNumber } = req.body;
    ...
    try {
        ...
        const updatedUser = await BenOrgMODEL.findByIdAndUpdate(id, {
            $set: {
                orgName,
                orgId,
                address,
...
const deleteBenOrg = async (req, res) => {
    const { id } = req.params
    try {
        const deleteUser = await BenOrgMODEL.findByIdAndUpdate(
            id,
            { isDeleted: true },
            { new: true }
        )
...
module.exports = {
    ...
    loginBenOrg,
    sendEmailBenOrg,
    verificationCodeBenOrg,
    updatepasswordBenOrg,
    getBenOrgData,
    getBenOrgId,
    updateBenOrgData,
    deleteBenOrg
